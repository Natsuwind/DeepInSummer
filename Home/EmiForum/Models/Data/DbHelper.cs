...
namespace Natsuhime.Data
{
    ...
    public class DbHelper
    {
        ...
        /// <summary>
        ///  数据库类型(SqlServer,Sqlite,Access,MySql)
        /// </summary>
        ...
        public static string ConnectionString
        {
            get
            {
                if (m_connectionstring == null)
                {
                    ...
                    {
                        m_connectionstring = "Data Source=" + System.Web.HttpContext.Current.Server.MapPath(DbConfigs.GetConfig().Dbconnectstring);
                    }
                    else
                    {
        ...
        public static string Dbtype
        {
            get
            {
                if (m_dbtype == null)
                {
                    m_dbtype = DbConfigs.GetConfig().Dbtype;
                }
                return DbHelper.m_dbtype;
            }
            set { DbHelper.m_dbtype = value; }
        }
                ...
                if (m_provider == null)
                {
                    lock (lockHelper)
                    {
                        if (m_provider == null)
                        {
                            try
                            {
                                m_provider = (IDbProvider)Activator.CreateInstance(Type.GetType(string.Format("Natsuhime.Data.DbProviders.{0}, EmiForum", Dbtype), false, true));
                            }
                            catch
                            {
