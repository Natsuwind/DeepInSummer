...
if exists (select * from dbo.sysobjects where id = object_id('dnt_attachpaymentlog') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
...
CREATE TABLE [dnt_debates] (
	...
	[positivevote] [int] NOT NULL CONSTRAINT [DF_dnt_debates_positivevote] DEFAULT (0),
	[positivevoterids] [text] NOT NULL CONSTRAINT [DF_dnt_debates_positivevoterids] DEFAULT (''),
	[negativevote] [int] NOT NULL CONSTRAINT [DF_dnt_debates_negativevote] DEFAULT (0),
	[negativevoterids] [text] NOT NULL CONSTRAINT [DF_dnt_debates_negativevoterids] DEFAULT (''),
...
CREATE TABLE [dnt_help] (
	...
	[message] [ntext] NULL ,
	[pid] [int] NOT NULL ,
	[orderby] [int] NOT NULL CONSTRAINT [DF_dnt_help_orderby] DEFAULT (0)
) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]
GO
...
IF OBJECT_ID('dnt_verifyreg') IS NOT NULL
DROP TABLE [dnt_verifyreg]
GO
...
IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'dnt_userconnect') AND OBJECTPROPERTY(id, N'IsUserTable') = 1)
...
IF NOT EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'dnt_pushfeedlog') AND OBJECTPROPERTY(id, N'IsUserTable') = 1)
...
IF OBJECT_ID('dnt_connectbindlog') IS NOT NULL
DROP TABLE [dnt_connectbindlog]
GO
