...
IF @@ERROR=0
    BEGIN
        ...
        BEGIN
            ...
            IF @layer>0
				UPDATE [dnt_topics] SET [replies]=[replies] + 1,[lastposter]=@poster,[lastpost]=@postdatetime,[lastposterid]=@posterid,[lastpostid]=@postid WHERE [tid]=@tid
        END
        
        IF @ishide=0 AND EXISTS(SELECT [hide] FROM [dnt_topics] WHERE [tid]=@tid AND [hide]=1)
			SET @ishide=1
			
        IF @layer<=0
        BEGIN
          UPDATE [dnt_topics] SET [replies]=0,[lastposter]=@poster,[lastpost]=@postdatetime,[lastposterid]=@posterid,[lastpostid]=@postid,[hide]=@ishide WHERE [tid]=@tid
        END
            IF @posterid <> -1
        BEGIN
...
SET @fidlist = '';
		...
		UPDATE [dnt_forums] SET [topics] = [topics] + 1,[curtopics] = [curtopics] + 1 WHERE (CHARINDEX(',' + RTRIM([fid]) + ',', ',' + (SELECT @fidlist AS [fid]) + ',') > 0)
                            ...
                            BEGIN
								...
								IF @chanageposts = 1
									BEGIN
										...
										UPDATE [dnt_forums] SET [posts]=[posts] - @postcount, [topics]=[topics] - 1,[todayposts]=[todayposts] - @todaycount WHERE [fid] IN (SELECT [item] FROM [dnt_split](@fidlist, ',')) 
...
FROM [dnt_posts1] 
				...
				(SELECT pid FROM (SELECT pid FROM dnt_posts1 WHERE tid='+@tid+' AND layer=0 UNION ALL SELECT PID FROM dnt_postdebatefields WHERE tid='+@tid+' AND opinion='+@opinion+') AS T)') 
	ELSE
		EXEC('SELECT  TOP ' + @pagesize + '
[dnt_posts1].[attachment],
[dnt_posts1].[bbcodeoff],
...
[dnt_userfields].[yahoo] FROM [dnt_posts1] 
				...
				LEFT JOIN [dnt_userfields] ON [dnt_userfields].[uid]=[dnt_users].[uid] 
				WHERE [dnt_posts1].invisible=0 AND [dnt_posts1].pid IN 
					(SELECT pid FROM dnt_postdebatefields 
					 WHERE opinion=' + @opinion + ' AND 
							tid=' + @tid+' AND pid > (SELECT MAX(pid) FROM (
								SELECT TOP ' + @pagetop + ' pid FROM (SELECT pid FROM dnt_posts1 WHERE tid='+@tid+' AND layer=0 UNION ALL SELECT PID FROM dnt_postdebatefields WHERE tid='+@tid+' AND opinion='+@opinion+') AS T ORDER BY pid )AS tblTmp))')
END
GO
...
[lastpostid],[lastposterid],[replies],[highlight],[digest],[displayorder],[attachment],[closed],[magic],[special],[identify] FROM 
...
IF @pageindex = 1
	BEGIN
		SET @strSQL = 'SELECT TOP ' + STR(@pagesize) + '[rate], [tid],[fid],[typeid],[iconid],[title],[price],[hide],[readperm], [special],[poster],[posterid],[views],[postdatetime],[lastpost],[lastposter],[lastpostid],[lastposterid],[replies],[highlight],[digest],[displayorder],[closed],[attachment],[magic],[identify] FROM [dnt_topics] WHERE [displayorder]>0 AND [tid] IN (SELECT [item] FROM [dnt_split]('''+@tids+''', '','')) ORDER BY [displayorder] DESC, [lastpost] DESC'
	END
ELSE
	BEGIN
		SET @strSQL = 'SELECT TOP ' + STR(@pagesize) + ' [rate], [tid],[fid],[typeid],[iconid],[title],[price],[hide],[readperm], [special],[poster],[posterid],[views],[postdatetime],[lastpost],[lastposter],[lastpostid],[lastposterid],[replies],[highlight],[digest],[displayorder],[closed],[attachment],[magic],[identify] FROM [dnt_topics]
WHERE (EXISTS   (SELECT TOP ' + STR((@pageindex-1)*@pagesize) + ' [tid]
                 FROM [dnt_topics]
                 WHERE [displayorder] > 0 AND [tid] IN (SELECT [item] FROM [dnt_split]('''+@tids+''', '',''))
                 ORDER BY [lastpost] DESC)) AND (displayorder > 0) AND 
      ([tid] IN (SELECT [item] FROM [dnt_split]('''+@tids+''', '',''))
...
CREATE PROCEDURE [dnt_getnewtopics1]
	...
	SELECT TOP 20 
        ...
        WHERE [dnt_forums].[fid] IN (SELECT [item] FROM [dnt_split](@fidlist, ','))
...
AS 
SET @olidlist = RTRIM(@olidlist)
DELETE FROM [dnt_online] WHERE [olid] IN (SELECT [item] FROM [dnt_split](@olidlist, ',')) 
GO
...
EXEC('SELECT TOP ' + @pagesize + '
									...
									[dnt_userfields].[yahoo], 
									[dnt_userfields].[skype] 
	 FROM [dnt_posts1] LEFT JOIN [dnt_users] ON [dnt_users].[uid]=[dnt_posts1].[posterid] LEFT JOIN [dnt_userfields] ON [dnt_userfields].[uid]=[dnt_users].[uid] WHERE [dnt_posts1].[tid]=' + @tid + ' AND [dnt_posts1].[invisible]<='+@invisible+' ORDER BY [dnt_posts1].[pid]')
...
EXEC('SELECT TOP ' + @pagesize + ' 
									...
									[dnt_userfields].[yahoo], 
									[dnt_userfields].[skype] 
	FROM [dnt_posts1] LEFT JOIN [dnt_users] ON [dnt_users].[uid]=[dnt_posts1].[posterid] LEFT JOIN [dnt_userfields] ON [dnt_userfields].[uid]=[dnt_users].[uid] WHERE [dnt_posts1].[tid]=' + @tid + ' AND [pid] > (SELECT MAX([pid]) FROM (SELECT TOP ' + @pagetop + ' [dnt_posts1].[pid] FROM [dnt_posts1] WHERE [dnt_posts1].[tid]=' + @tid + ' AND [dnt_posts1].[invisible]<='+@invisible+' ORDER BY [dnt_posts1].[pid]) AS tblTmp) AND [dnt_posts1].[invisible]<='+@invisible+' ORDER BY [dnt_posts1].[pid]')
GO
			    ...
			    END	
                    ...
                    IF @chanageposts = 1
	                    BEGIN
		                    ...
		                    EXEC('UPDATE [dnt_forums] SET [posts]=[posts]- 1,  [topics]=[topics] - 1, [todayposts] =[todayposts]-1  WHERE [fid] = IN ('+@fid+')')
	                    END
...
IF OBJECT_ID('dnt_createverifyreg','P') IS NOT NULL
DROP PROC [dnt_createverifyreg]
GO

CREATE PROCEDURE [dnt_createverifyreg]
	@ip char(15),
	@email char(50),
	@createtime smalldatetime,
	@expiretime smalldatetime,
	...
	@verifycode nchar(16)
AS
	INSERT INTO [dnt_verifyreg]([ip],[email],[createtime],[expiretime],[invitecode],[verifycode]) 
	VALUES(@ip,@email,@createtime,@expiretime,@invitecode,@verifycode)
GO

IF OBJECT_ID('dnt_deleteverifyreg','P') IS NOT NULL
DROP PROC [dnt_deleteverifyreg]
GO

CREATE PROCEDURE [dnt_deleteverifyreg]
	@regid int
AS
	DELETE [dnt_verifyreg] WHERE [regid]=@regid
GO

IF OBJECT_ID('dnt_getverifyreg','P') IS NOT NULL
DROP PROC [dnt_getverifyreg]
GO

CREATE PROCEDURE [dnt_getverifyreg]
	@field nchar(50),
	@type nchar(10)
AS
	SET NOCOUNT ON;
	IF @type='code'
		SELECT [regid],[ip],[email],[createtime],[expiretime],[invitecode],[verifycode] FROM [dnt_verifyreg] WHERE [verifycode]=@field 
	ELSE IF @type='ip'
		SELECT TOP 1 [regid],[ip],[email],[createtime],[expiretime],[invitecode],[verifycode] FROM [dnt_verifyreg] WHERE [ip]=@field ORDER BY [createtime] DESC
	ELSE IF @type='email'
		SELECT TOP 1 [regid],[ip],[email],[createtime],[expiretime],[invitecode],[verifycode] FROM [dnt_verifyreg] WHERE [email]=@field  ORDER BY [createtime] DESC
GO

IF OBJECT_ID('dnt_getmyunaudittopic','P') IS NOT NULL
DROP PROC [dnt_getmyunaudittopic]
GO

CREATE PROCEDURE [dnt_getmyunaudittopic]
@posterid int,
@pagesize int,
@displayorder INT,
@pageindex int
...
IF @pageindex = 1
	BEGIN
		EXEC('SELECT TOP '+ @pagesize+' [rate], [fid], [tid],[iconid],[typeid],[title],[price],[hide],[readperm],[poster],[posterid],[replies],[views],[postdatetime],[lastpost],[lastposter],[lastpostid],[lastposterid],[highlight],[digest],[displayorder],[attachment],[closed],[magic],[special] FROM [dnt_topics] WHERE [posterid]=' +@posterid + ' AND [displayorder]= '+@displayorder+' ORDER BY [tid] DESC')
END
ELSE
	BEGIN
		EXEC('SELECT TOP '+ @pagesize+' [rate], [fid], [tid],[iconid],[typeid],[title],[price],[hide],[readperm],[poster],[posterid],[replies],[views],[postdatetime],[lastpost],[lastposter],[lastpostid],[lastposterid],[highlight],[digest],[displayorder],[attachment],[closed],[magic],[special] FROM [dnt_topics] WHERE [posterid]=' +@posterid + ' AND [displayorder]= '+@displayorder+' AND tid < (SELECT MIN(tid) FROM (SELECT TOP '+@startRow+' tid FROM [dnt_topics] WHERE [posterid]=' +@posterid+' AND displayorder='+@displayorder+') AS T) ORDER BY tid DESC')
END
GO

IF OBJECT_ID('dnt_createuserconnect','P') IS NOT NULL
DROP PROC [dnt_createuserconnect]
GO

CREATE PROCEDURE [dnt_createuserconnect]
	@openid char(32),
	@uid int,
	@token char(16),
	@secret char(16),
	@allowvisitqquserinfo int,
	@allowpushfeed int,
	@issetpassword int,
	@callbackinfo nvarchar(100)
AS
	INSERT INTO [dnt_userconnect] ([openid],[uid],[token],[secret],[allowvisitqquserinfo],[allowpushfeed],[issetpassword],[callbackinfo])
	VALUES(@openid,@uid,@token,@secret,@allowvisitqquserinfo,@allowpushfeed,@issetpassword,@callbackinfo)
GO

IF OBJECT_ID('dnt_getuserconnect','P') IS NOT NULL
DROP PROC [dnt_getuserconnect]
GO

CREATE PROCEDURE [dnt_getuserconnect]
	@openid char(32)
AS
	SET NOCOUNT ON;
	SELECT [openid],[uid],[token],[secret],[allowvisitqquserinfo],[allowpushfeed],[issetpassword],[callbackinfo] FROM [dnt_userconnect] WHERE [openid]=@openid
GO

IF OBJECT_ID('dnt_updateuserconnect','P') IS NOT NULL
DROP PROC [dnt_updateuserconnect]
GO

CREATE PROCEDURE [dnt_updateuserconnect]
	@openid char(32),
	@uid int,
	@token char(16),
	@secret char(16),
	...
	@callbackinfo nvarchar(100)
AS
	UPDATE [dnt_userconnect] SET [uid]=@uid,[token]=@token,[secret]=@secret,
	[allowvisitqquserinfo]=@allowvisitqquserinfo,[allowpushfeed]=@allowpushfeed ,[issetpassword]=@issetpassword,[callbackinfo]=@callbackinfo
	WHERE [openid]=@openid
GO

IF OBJECT_ID('dnt_getuserconnectbyuid','P') IS NOT NULL
DROP PROC [dnt_getuserconnectbyuid]
GO

CREATE PROCEDURE [dnt_getuserconnectbyuid]
	@uid int
AS
	SET NOCOUNT ON;
	SELECT [openid],[uid],[token],[secret],[allowvisitqquserinfo],[allowpushfeed],[issetpassword],[callbackinfo] FROM [dnt_userconnect] WHERE [uid]=@uid
GO

IF OBJECT_ID('dnt_deleteuserconnect','P') IS NOT NULL
DROP PROC [dnt_deleteuserconnect]
GO

CREATE PROCEDURE [dnt_deleteuserconnect]
	@type char(10),
	@openid char(32),
	@uid int
AS
	IF @type='uid'
		DELETE [dnt_userconnect] WHERE [uid]=@uid
	ELSE
		DELETE [dnt_userconnect] WHERE [openid]=@openid
GO

IF OBJECT_ID('dnt_createfeedlog','P') IS NOT NULL
DROP PROC [dnt_createfeedlog]
GO

CREATE PROCEDURE [dnt_createfeedlog]
	@tid int,
	@uid int,
	@token char(16),
	@secret char(16)
AS
	INSERT INTO [dnt_pushfeedlog]([tid],[uid],[authortoken],[authorsecret]) VALUES(@tid,@uid,@token,@secret)
GO

IF OBJECT_ID('dnt_getfeedlog','P') IS NOT NULL
DROP PROC [dnt_getfeedlog]
GO

CREATE PROCEDURE [dnt_getfeedlog]
	@tid int
AS
	SET NOCOUNT ON;
	SELECT [tid],[uid],[authortoken],[authorsecret] FROM [dnt_pushfeedlog] WHERE [tid]=@tid
GO

IF OBJECT_ID('dnt_deletefeedlog','P') IS NOT NULL
DROP PROC [dnt_deletefeedlog]
GO

CREATE PROCEDURE [dnt_deletefeedlog]
	@tid int
AS
	DELETE [dnt_pushfeedlog] WHERE [tid]=@tid
GO

IF OBJECT_ID('dnt_createbindlog','P') IS NOT NULL
DROP PROC [dnt_createbindlog]
GO

CREATE PROCEDURE [dnt_createbindlog]
	@openid char(32),
	@uid int
AS
BEGIN
	INSERT INTO [dnt_connectbindlog]([openid],[uid],[type],[bindcount])
	VALUES(@openid,@uid,1,1)
END
GO

IF OBJECT_ID('dnt_updatebindlog','P') IS NOT NULL
DROP PROC [dnt_updatebindlog]
GO

CREATE PROCEDURE [dnt_updatebindlog]
	@openid char(32),
	@uid int,
	@type smallint,
	@bindcount smallint
AS
BEGIN
	UPDATE [dnt_connectbindlog] SET [uid]=@uid,[type]=@type,[bindcount]=@bindcount WHERE [openid]=@openid
END
GO

IF OBJECT_ID('dnt_getbindlog','P') IS NOT NULL
DROP PROC [dnt_getbindlog]
GO

CREATE PROCEDURE [dnt_getbindlog]
	@openid char(32)
AS
	SET NOCOUNT ON;
	SELECT [openid],[uid],[type],[bindcount] FROM [dnt_connectbindlog] WHERE openid=@openid