...
function addAttach(prefix, filetype) {
	...
	for (var index = 0; index < uploadAttachFiles.length; index++) {
	    ...
	    for (i in tags) {
	        if (tags[i].id == prefix + type + 'localfile[]') {
	            tags[i].id = prefix + type + 'localfile_' + id;
	        } else if (tags[i].id == prefix + type + 'cpdel[]') {
	            tags[i].id = prefix + type + 'cpdel_' + id;
	        } else if (tags[i].id == prefix + type + 'localno[]') {
	            tags[i].id = prefix + type + 'localno_' + id;
	        } else if (tags[i].id == prefix + type + 'deschidden[]') {
	            tags[i].id = prefix + type + 'deschidden_' + id;
	        }
	    }
	    AID++;
...
function getattachlist_callback(doc) {
    ...
    if (data.length > 0) {
        ...
        for (i = 0; i < data.length; i++) {
            ...
            html += '<td class="attp"><input type="text" name="attachprice_' + data[i].aid + '" value="0" size="1" onblur="checkmaxpric(this);"></td>';
...
function checkmaxpric(inputobj) {
    var attachpricec = inputobj.value;

    if (attachpricec > maxprice) {
        alert("系统规定附件最高售价为" + maxprice);
        return;
    }
    if (attachpricec < 0) {
        alert("售价不可小于0");
...
function delSimpleAttach(id, type) {
    data = id.toString().split(",")
    _sendRequest('tools/ajax.aspx?t=deleteattach&aid=' + data[0] + '&fid=' + data[1], delnouseSimpleAttach_callback, false);
}
