...
AND [dnt_posts1].pid IN (SELECT pid FROM [dnt_posts1] WHERE [dnt_posts1].tid=@tid and [dnt_posts1].layer=0 UNION ALL SELECT pid FROM dnt_postdebatefields WHERE tid=@tid AND opinion=@opinion)) AS DEBATEPOST
...
[dnt_topics].[identify]
FROM [dnt_topics]
WHERE  [tid] IN (SELECT [item] FROM [dnt_split](@tids, ',')) AND [displayorder]>0 )AS TOPICS
WHERE ROWID BETWEEN @startRow AND @endRow 
GO
...
WHERE [tid] = @tid AND [dnt_posts1].[invisible] <=@invisible) AS POSTS 
WHERE ROWID BETWEEN @startRow AND @endRow
GO
