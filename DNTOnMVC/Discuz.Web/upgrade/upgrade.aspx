<%@ Page Language="c#" Inherits="Discuz.Install.Upgrade" %>
...
<body>
    <div class="wrap cl">
        <h2><img alt="Discuz!NT|BBS|论坛" src="images/logo.png"><cite><b><%=Discuz.Common.Utils.ASSEMBLY_VERSION%></b>升级程序</cite></h2>
        <div class="nav">
            <ul>
                ...
                <li <%if(stepNum==2){ %> class="cur"<%}else if(stepNum>2){ %> class="finish"<%} %>><span>升级</span></li>
            </ul>
	    </div>
        <%if(stepNum==0){ %>
        <div class="main cl">
            ...
            <div class="inner">
                ...
                <p>北京康盛新创科技有限责任公司（Comsenz Technology Ltd）为 Discuz!NT 产品的开发商，依法独立拥有 Discuz!NT 产品著作权（中国国家版权局著作权登记号
                    2003SR6623）。北京康盛新创科技有限责任公司网址为 <a href="http://www.comsenz.com" target="_blank">http://www.comsenz.com</a>，
                    Discuz!NT 官方讨论区网址为 <a href="http://nt.discuz.net" target="_blank">http://nt.discuz.net</a>。</p>
                <p>Discuz!NT 著作权已在中华人民共和国国家版权局注册，著作权受到法律和国际公约保护。使用者：无论个人或组织、盈利与否、用途如何（包括以学习和研究为目的），均需仔细阅读本协议，在理解、同意、并遵守本协议的全部条款后，方可开始使用
                    Discuz!NT 软件。</p>
                ...
                <br />
                <br />
                3) 无论如何，即无论用途如何、是否经过修改或美化、修改程度如何，只要使用 Discuz!NT 的整体或任何部分，未经书面许可，论坛页面页脚处的 Discuz!NT
                名称和北京康盛新创科技有限责任公司下属网站（http://www.comsenz.com、http://www.discuznt.com 或 http://nt.discuz.net）
                的链接都必须保留，而不能清除或修改。<br />
                <br />
                ...
                <p>电子文本形式的授权协议如同双方书面签署的协议一样，具有完全的和等同的法律效力。您一旦开始安装 Discuz!NT，即被视为完全理解并接受本协议的各项条款，在享有上述条款授予的权力的同时，受到相关的约束和限制。协议许可范围以外的行为，将直接违反本授权协议并构成侵权，我们有权随时终止授权，责令停止损害，并保留追究相关责任的权力。
                    <br />
                    <br />
                </p>
                <h2>附：</h2>
        ...
        <div class="btn cl">
            <a href="javascript:window.close();" class="back">退出升级</a>
            <a href="upgrade.aspx?step=servertest" class="next">接受协议</a>
        </div>
        <%}else if(stepNum==1){ %>
	    <div class="main cl">
		<h1>环境检测</h1>
		...
		<a id="nextlink" href="#" class="back">下一步</a>		
	    </div>
        ...
        for (i = 0; i < resultlist.length; i++) {
            ...
            if (resultlist[i].result == 'true') {
                icon = 'ok';
                iconname = '成功';
                $('#resultlist').append('<li><a href="#">' + resultlist[i].msg + '</a></li>');
            }
            else {
                icon = 'error';
                iconname = '失败';
                $('#resultlist').append('<li class="error"><a href="http://faq.comsenz.com/viewnews-984" target="_blank">' + resultlist[i].msg + '</a></li>');
                errorcount++;
            }
        }
        var dbCheckResult = <%CheckConnection();%>;
        if(dbCheckResult.Result){
            icon = "ok";
            iconname = "成功";
            $('#resultlist').append('<li><a href="#">' + dbCheckResult.Message + '</a></li>');
        }
        else{
            icon = "error";
            iconname = "失败";
            errorcount++;
            $('#resultlist').append('<li class="error"><a href="#">' + dbCheckResult.Message + '</a></li>');
        }
        if(errorcount <= 0){
            $('#nextlink').attr("href","upgrade.aspx?step=upgrade").addClass("next");
        }
    </script>
        <%}else if(stepNum==2){ %>
        <div class="main cl">
        ...
        <script type="text/javascript">
            function showmessage(message) {
                $('#processlist').append('<li>' + message + '</li>');
                $(".inner").get(0).scrollTop = 100000000;
            }
            function showerrormessage(message) {
                $('#processlist').append('<li class="error">' + message + '</li>');
                $(".inner").get(0).scrollTop = 100000000;
            }
        </script>
        <%UpgradeProcess();%>
        <%}%>
        <div class="copy">
            北京康盛新创科技有限责任公司 &copy; 2001 - <%=DateTime.Now.Year%> Comsenz Inc.
        </div>
    </div>
</body>
</html>
