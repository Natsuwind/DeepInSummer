...
<script type="text/javascript">
    var checkConnection = function () {
        $("#loading").ajaxStart(function () {
            $("#loading").removeClass("hint_info").addClass("loading_info").css("color", "").html("正在检测数据库链接...");
        });

        $.getJSON("step2.aspx?connection=check&r=" + Math.random(), function (data) {
            if (!data.Result) {
                $("#loading").removeClass("loading_info").addClass("hint_info").css("color", "#f00").html(data.Message);
            }
            else {
    ...
    function upgrade() {
        ...
        $.getJSON("step2.aspx?upgrade=true&r=" + Math.random(), $('#form1').serialize(), function (data) {
            if (data.Result) {
                ...
                showbox.hide();
                showbox = new Boxy('<p style=\"width:300px;font-size:14px;\">' + data.Message + '<br /><a href=\'#\'>重试</a></p>', { closeable: false, modal: true, center: true });
                showbox.show();
            }
        });
        return false;
...
<div class="wrap cl">
    ...
    <form id="form1" runat="server" >
	    ...
	    <div class="btn cl">
		    <a href="#" id="recheck" class="back">重新检测</a>
		    <a href="#" id="upgrade" class="next">升级</a>
	    </div>
    </form>
	<div class="copy">
