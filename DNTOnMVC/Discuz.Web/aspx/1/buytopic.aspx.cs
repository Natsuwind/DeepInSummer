...
namespace Discuz.Web
{
    ...
    public class buytopic : TopicPage
    {
        ...
        protected override void ShowPage()
        {
            ...
            {
                price = topic.Price;
                expirehours = Utils.StrDateDiffHours(topic.Postdatetime, maxchargespan);
                //判断当前用户是否已经购买
                if (PaymentLogs.IsBuyer(topicid, userid) || (expirehours > 0 && maxchargespan != 0))
                    price = -1;
                else
        ...
        private bool SetTopicInfo()
        {
            ...
            if (((topic.Posterid == userid || ismoder == 1) && DNTRequest.GetString("showpayments") == "") || topic.Price <= 0)
            {
