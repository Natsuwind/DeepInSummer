...
namespace Discuz.Web
{
    ...
    public class showtopic : Discuz.Web.UI.TopicPage
    {
        ...
        /// <summary>
        /// 查看辩论，0查看全部，1查看正方，2查看反方
        /// </summary>
        public int stand = DNTRequest.GetInt("stand", 0);
        /// <summary>
        /// 获取url中的invisible,1为不显示,0为显示,-2为屏蔽
        /// </summary>
        ...
        public new void BindPageCountAndId()
        {
            ...
            string typeidParam = typeid == -1 ? "" : "&typeid=" + typeid;
            string standParam = stand == 0 ? "" : "&stand=" + stand;
            string onlyauthorParam = string.IsNullOrEmpty(onlyauthor) || onlyauthor == "0" ? "" : "onlyauthor=" + onlyauthor + "&posterid=" + posterid;
            //得到页码链接
            if (Utils.StrIsNullOrEmpty(onlyauthor) || onlyauthor == "0")
            {
                if (config.Aspxrewrite == 1 && typeid <= -1 && stand == 0)
                    ...
                    pagenumbers = Utils.GetPageNumbers(pageid, pagecount, string.Format("showtopic.aspx?topicid={0}{1}{2}", topicid, typeidParam, standParam), 8);
            }
            else
                pagenumbers = Utils.GetPageNumbers(pageid, pagecount, string.Format("showtopic.aspx?topicid={0}&{1}{2}", topicid, onlyauthorParam, typeidParam), 8);

            if (pageid != pagecount)
            {
                if (stand != 0)
                {
                    nextpage = string.Format("<a href=\"showtopic.aspx?topicid={0}{1}{2}&page={3}\" class=\"next\">下一页</a>", topicid, typeidParam, standParam, pageid + 1);
                }
                else
                {
                    string showtopiclink = Urls.ShowTopicAspxRewrite(topicid, pageid + 1, typeid);
                    string splitFlag = onlyauthorParam == "" ? "" : (showtopiclink.IndexOf("?") == -1 ? "?" : "&");
                    nextpage = "<a href=\"" + showtopiclink + splitFlag + onlyauthorParam + "\" class=\"next\">下一页</a>";
                }
            }
            ...
            if (onlyauthor == "" || onlyauthor == "0")
                ...
                ForumUtils.WriteCookie("referer", "showtopic.aspx?&topicid=" + topicid + "&" + onlyauthorParam + "&page=" + pageid);
        }
    }
}
