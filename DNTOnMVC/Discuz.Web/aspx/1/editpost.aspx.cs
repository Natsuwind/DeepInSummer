...
namespace Discuz.Web
{
    ...
    public class editpost : PageBase
    {
        ...
        protected override void ShowPage()
        {
            ...
            else if (admininfo != null && admininfo.Alloweditpost == 1 && Moderators.IsModer(useradminid, userid, forumid))
                alloweditpost = true;
            if (!alloweditpost && postinfo.Posterid != userid)
            {
            ...
            if (ispost)
            {
                ...
                CreditsFacade.UpdateUserCredits(userid);

                #region 设置提示信息和跳转链接
                string url = "";
                //辩论地址
                if (topic.Special == 4)
                    url = Urls.ShowTopicAspxRewrite(topic.Tid, pageid);
                else if (DNTRequest.GetQueryString("referer") != "")//ajax快速回复将传递referer参数
                    url = string.Format("showtopic.aspx?page=end&forumpage={2}&topicid={0}#{1}", topic.Tid, postinfo.Pid, forumpageid);
                else if (pageid > 1)//如果不是ajax,则应该是带pageid的参数
                {
                    if (config.Aspxrewrite == 1)
                        url = string.Format("showtopic-{0}-{2}{1}#{3}", topic.Tid, config.Extname, pageid, postinfo.Pid);
                    else
                        url = string.Format("showtopic.aspx?topicid={0}&forumpage={3}&page={2}#{1}", topic.Tid, postinfo.Pid, pageid, forumpageid);
                }
                else//如果都为空.就跳转到第一页(以免意外情况)
                {
                    if (config.Aspxrewrite == 1)
                        url = string.Format("showtopic-{0}{1}", topic.Tid, config.Extname);
                    else
                        url = string.Format("showtopic.aspx?topicid={0}&forumpage={1}", topic.Tid, forumpageid);
                }
                SetUrl(url);
        ...
        private void SetPostInfo(AdminGroupInfo admininfo, ShortUserInfo user, bool ishtmlon)
        {
            ...
            postinfo.Invisible = (postinfo.Invisible == 0 & needaudit) ? 1 : postinfo.Invisible;
        ...
        protected string AttachmentList()
        {
            ...
            foreach (DataRow dr in attachmentlist.Rows)
            {
                ...
                sb.AppendFormat("{{\\'aid\\':{0},\\'uid\\':{1},\\'attachment\\':\\'{2}\\',\\'attachkey\\':\\'{3}\\',\\'description\\':\\'{4}\\'}},",
                    dr["aid"].ToString(), userid.ToString(), dr["attachment"].ToString().Trim().Replace("'", "\\'"),Thumbnail.GetKey(TypeConverter.ObjectToInt(dr["aid"])), dr["description"].ToString().Trim().Replace("'", "\\'"));
            }
            return sb.ToString().TrimEnd(',') + "]";
        }

        //private string GetKey(string aid, string uid)
        //{
        //    //return Discuz.Common.DES.Encode(aid.ToString() + ",300,300", Utils.MD5(aid)).Replace("+", "[");
        //    return DES.Encode(string.Format("{0},{1},300,300", aid, uid), Utils.MD5(aid)).Replace("+", "[");
        //}
    }
}
