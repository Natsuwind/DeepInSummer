...
namespace Discuz.Web
{
    ...
    public class posttopic : PageBase
    {
        ...
        /// 提交的发送状态
        /// </summary>
        public int feedstatus = DNTRequest.GetInt("ispublishfeed", 0) + DNTRequest.GetInt("ispublisht", 0);
        #endregion
        ...
        protected override void ShowPage()
        {
            ...
            if (ispost)
            {
                ...
                //如果用户已绑定QQ,且勾选了发送动态
                if (isbindconnect && feedstatus > 0)
                {
                    PushFeed pushFeed = new PushFeed();
                    pushFeed.TopicPushFeed(topicinfo, postinfo, attachmentinfo, feedstatus);
                }
                ...
                else
                {
                    ...
                    //else
                    //    PostTopicSucceed(Forums.GetValues(forum.Postcredits), topicinfo, topicinfo.Tid);
                    if ((useradminid != 1) && (UserAuthority.NeedAudit(forum, useradminid, userid, usergroupinfo) || topicinfo.Displayorder == -2))
                    {
                        //是否需要审核
                        ForumUtils.WriteCookie("postmessage", "");
                        SetLastPostedForumCookie();
                        ...
                        PostTopicSucceed(forum, topicinfo);
                }
                #endregion
