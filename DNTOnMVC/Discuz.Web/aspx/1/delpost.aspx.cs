    ...
    public class delpost : PageBase
    {
        ...
        private bool CheckPermission(PostInfo post, int opinion)
        {
            ...
            if (userid == post.Posterid)
            {
                ...
                bool isModer = Moderators.IsModer(useradminid, userid, forumid);
                if (!isModer && ((config.Deletetimelimit != 0 && Utils.StrDateDiffMinutes(post.Postdatetime, config.Deletetimelimit) > 0) || post.Posterid != userid))   //不是作者或者超过编辑时限,0为不限时
                {
