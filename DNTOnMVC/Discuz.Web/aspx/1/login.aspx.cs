        ...
        private void RedirectAPILogin(ApplicationInfo appInfo)
        {
            ...
            if (Utils.StrIsNullOrEmpty(expires))
                expireUTCTime = DateTime.Parse(Users.GetShortUserInfo(userid).Lastvisit).ToUniversalTime().AddSeconds(
                    Convert.ToDouble(Utils.GetCookie("dnt", "expires")));
            else
            ...
            //应用程序登录需要提前更新当前用户的状态，并返回authToken
            olid = OnlineUsers.GetOlidByUid(userid);

            if (olid > 0)
                oluserinfo = OnlineUsers.GetOnlineUser(olid);
            ...
            System.Web.HttpContext.Current.Response.Redirect(string.Format("{0}{1}auth_token={2}{3}", appInfo.CallbackUrl, appInfo.CallbackUrl.IndexOf("?") > 0 ? "&" : "?", authToken, next == "" ? next : "&next=" + next));
        }
