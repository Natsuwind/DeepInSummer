            ...
            if (postid != -1)
            {
                ...
                if (!string.IsNullOrEmpty(DNTRequest.GetString("quote")))
                {
                    ...
                    //message = "[quote] 原帖由 [b]" + postinfo.Poster + "[/b] 于 " + postinfo.Postdatetime + " 发表\r\n ***隐藏帖*** [/quote]";
                      ...
                      , UBB.ClearAttachUBB(Utils.GetSubString(Regex.Replace(postinfo.Message, @"\[quote\](.|\n)+\[/quote\]", "", RegexOptions.Multiline), 200, "......")), postinfo.Poster, postinfo.Postdatetime, topicid, postid, Utils.GetRootUrl(forumpath));
                }
            }
