...
namespace Discuz.Web.UI
{
    ...
    public class SessionAjaxPage : PageBase
    {
        ...
        private void Detatevote()
        {
            ...
            string positiveVoterIDList = Debates.CheckDebateVoter(tid, 1);
            string negativeVoterIDList = Debates.CheckDebateVoter(tid, 2);

            if (Utils.IsEqualStr(userid.ToString(), positiveVoterIDList, ",") || Utils.IsEqualStr(userid.ToString(), negativeVoterIDList, ","))
            {
                StringBuilder xml = new StringBuilder();
                xml.Append("<root>您已投过票</root>");
                ResponseXML(xml);
            }
            else 
            {
                string voterIDList = stand == 1 ? positiveVoterIDList : negativeVoterIDList;
                voterIDList += ("," + userid.ToString());
                Debates.InsertDebateVoter(tid, voterIDList.Trim(','), stand);
                StringBuilder xml = new StringBuilder();
                xml.Append("<root>投票成功</root>");
                ResponseXML(xml);
            }
        }
        ...
        private void QuickReply()
        {
            ...
            {
                ResponseXML(xmlnode.Append("<error>主题尚未通过审核, 不能执行回复操作!</error>"));
                return;
            }
            #endregion
            ...
            if (!string.IsNullOrEmpty(attachId))
            {
                ...
                Attachments.UpdateAttachment(attachmentinfo, topic.Tid, postid, postinfo, ref sb, userid, config, usergroupinfo);
            }
            #endregion
        ...
        private StringBuilder GetNewPostXML(StringBuilder xmlnode, PostInfo postinfo, ForumInfo forum, TopicInfo topic, int postid)
        {
            ...
            postPramsInfo.CurrentUserGroup = UserGroups.GetUserGroupInfo(userInfo.Groupid);
            postPramsInfo.Usercredits = userInfo == null ? 0 : userInfo.Credits;

            StringBuilder message = new StringBuilder(UBB.UBBToHTML(postPramsInfo),3000);
            Regex regexAttach = new Regex(@"\[attach\](\d+?)\[\/attach\]", RegexOptions.IgnoreCase);
            Regex regexHide = new Regex(@"\s*\[hide\][\n\r]*([\s\S]+?)[\n\r]*\[\/hide\]\s*", RegexOptions.IgnoreCase);
            Regex regexAttachImg = new Regex(@"\[attachimg\](\d+?)\[\/attachimg\]", RegexOptions.IgnoreCase);
            List<ShowtopicPageAttachmentInfo> attachList = Attachments.GetAttachmentList(postPramsInfo, postinfo.Pid.ToString());
            postinfo.Attachment = attachList.Count;
            if (postinfo.Attachment > 0 || regexAttach.IsMatch(message.ToString()) || regexAttachImg.IsMatch(message.ToString()))
            {
                //获取在[hide]标签中的附件id
                ...
                {
                    message = new StringBuilder(Attachments.GetMessageWithAttachInfo(postPramsInfo, allowGetAttach, attHidArray, info, attach, message.ToString()), 300);
                ...
                postinfo.Message = message.ToString();
                if (attachList.Count > 0)
                {
                    message.Append("<div class=\"postattachlist\">" +
                    ...
                    foreach (ShowtopicPageAttachmentInfo attachment in attachList)
                    {
                        if (attachment.Attachimgpost == 1)
                            message.Append("<dl class=\"t_attachlist_img attachimg cl\">");
                        else
                            message.Append("<dl class=\"t_attachlist attachimg cl\">");
                        if (attachment.Attachimgpost == 1)
                            message.Append("<dt></dt>");
                        else if (attachment.Filename.Trim().ToLower().EndsWith("rar") || attachment.Filename.Trim().ToLower().EndsWith("zip"))
                            message.Append("<dt><img class=\"absmiddle\" border=\"0\" src=\"images/attachicons/rar.gif\"/></dt>");
                        else
                            message.Append("<dt><img class=\"absmiddle\" border=\"0\" src=\"images/attachicons/attachment.gif\"/></dt>");
                        message.AppendFormat("<dd><a target=\"_blank\" onclick=\"return ShowDownloadTip({0}"+
                            ");\" href=\"attachment.aspx?attachmentid={1}\" class=\"xg2\">{2}</a><em class=\"xg1\">(<script type=\"text/javascript\">ShowFormatBytesStr({3});</script>, 下载次数:{4})</em>" +
                            "<p><span style=\"color:#666\">({5} 上传)</span></p>", postinfo.Posterid, attachment.Aid, attachment.Attachment, attachment.Filesize, attachment.Downloads, attachment.Postdatetime);
                        if (attachment.Preview != "")
                            message.AppendFormat("<p>{0}</p>", attachment.Preview);
                        message.AppendFormat("<p><a name=\"attach{0}\"></a>", attachment.Aid);
                        if(config.Showimages == 1 && attachment.Attachimgpost == 1)
                            if (config.Showimgattachmode == 0)
                            {
                                message.AppendFormat("<img imageid=\"{0}\" alt=\"{1}\" ", attachment.Aid, attachment.Attachment);
                                if (config.Showattachmentpath == 1)
                                    message.AppendFormat("src=\"{0}\"", attachment.Filename.IndexOf("http") >= 0 ? attachment.Filename : "attachment.aspx?attachmentid=" + attachment.Aid);
                                else
                                    message.AppendFormat("src=\"attachment.aspx?attachmentid={0}\"", attachment.Aid);
                                message.Append(" onmouseover=\"attachimg(this, 'mouseover')\" onload=\"attachimg(this, 'load');\" onclick=\"zoom(this, this.src);\" />");
                            }
                            else
                            {
                                message.AppendFormat("<img imageid=\"{0}\" alt=\"点击加载图片\" ", attachment.Aid);
                                if (config.Showattachmentpath == 1)
                                    message.AppendFormat("newsrc=\"{0}\"", attachment.Filename.IndexOf("http") >= 0 ? attachment.Filename : "upload/" + attachment.Filename);
                                else
                                    message.AppendFormat("newsrc=\"attachment.aspx?attachmentid={0}\"", attachment.Aid);
                                message.Append("src=\"/images/common/imgloading.png\" onload=\"attachimg(this, 'load');\" onclick=\"loadImg(this);\" />");
                            }
                        message.Append("</p></dd></dl>");
                    }
                }
            }
            ...
            //头衔、星星
            UserGroupInfo tmpUserGroupInfo = UserGroups.GetUserGroupInfo(Utils.StrToInt(usergroupid, CreditsFacade.GetCreditsUserGroupId(Utils.StrToInt(userinfo.Credits.ToString(), 0)).Groupid));
