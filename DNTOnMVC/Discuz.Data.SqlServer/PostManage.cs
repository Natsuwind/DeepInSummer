...
namespace Discuz.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public int GetMyUnauditTopicCount(int posterId, int filter)
        {
            string commandText = string.Format("SELECT COUNT(1) FROM [{0}topics] WHERE [posterid]=@posterid AND [displayorder]=@filter",
                                                BaseConfigs.GetTablePrefix);
        ...
        public IDataReader GetMyUnauditTopic(int posterId, int tpp, int pageId, int filter)
        {
            DbParameter[] parms = {
                                        DbHelper.MakeInParam("@posterid", (DbType)SqlDbType.Int, 4, posterId),
                                        DbHelper.MakeInParam("@displayorder", (DbType)SqlDbType.Int, 4, filter),
            ...
            return DbHelper.ExecuteReader(CommandType.StoredProcedure, string.Format("{0}getmyunaudittopic", BaseConfigs.GetTablePrefix), parms);
        }
        ...
        public int GetDebatePostCount(string onlyauthor, string postTableId, int tid, int posterId, int stand)
        {
            DbParameter[] parms = {
									   DbHelper.MakeInParam("@tid",(DbType)SqlDbType.NChar,100,tid),
									   DbHelper.MakeInParam("@posterid", (DbType)SqlDbType.VarChar, 20, posterId),
                                       DbHelper.MakeInParam("@opinion",(DbType)SqlDbType.Int,4,stand)
								   };
            string commandText = string.Format("SELECT COUNT(pid) FROM [{0}posts{1}] WHERE [tid] = @tid{2} AND [layer]>=0 AND [pid] IN (SELECT [pid] FROM [{0}postdebatefields] WHERE [tid]=@tid AND [opinion]=@opinion)",
                BaseConfigs.GetTablePrefix, postTableId, onlyauthor == "" || onlyauthor == "0" ? "" : " AND [posterid] = @posterid");
        ...
        {
            bool isHide = Regex.IsMatch(postInfo.Message, @"\s*(\[hide\][\n\r]*([\s\S]+?)[\n\r]*\[\/hide\]|\[hide=(\d+?)\][\n\r]*([\s\S]+?)[\n\r]*\[\/hide\])\s*",
            ...
            DbParameter[] parms = {
                                       ...
                                       DbHelper.MakeInParam("@ishide",(DbType)SqlDbType.Int,4,isHide ? 1 : 0)
								   };
        ...
        /// <returns></returns>
        public IDataReader GetPostList(string topicList, string[] postTableId)
        {
            ...
            return DbHelper.ExecuteReader(CommandType.Text, stringBuilder.ToString());
        }
        ...
        public IDataReader GetPostList(PostpramsInfo postParmsInfo, string postTableId)
        {
            DbParameter[] parms = {
                                           ...
                                           DbHelper.MakeInParam("@invisible",(DbType)SqlDbType.Int,4,postParmsInfo.Invisible),
									   };
        ...
        public string CheckDebateVoter(int tid, int stand)
        {
            DbParameter parm = DbHelper.MakeInParam("@tid", (DbType)SqlDbType.Int, 4, tid);
            return DbHelper.ExecuteScalarToStr(CommandType.Text, string.Format("SELECT [{0}] FROM [{1}debates] WHERE tid = @tid", stand == 1 ? "positivevoterids" : "negativevote", BaseConfigs.GetTablePrefix), parm);
        }

        public void InsertDebateVoter(int tid, string uidList, int stand)
        {
            DbParameter[] parms = {
                                    DbHelper.MakeInParam("@tid",(DbType)SqlDbType.Int,4,tid),
                                    DbHelper.MakeInParam("@voteridlist",(DbType)SqlDbType.Text,0,uidList)
                                };
            if (stand == 1)
                DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("update {0}debates set positivevoterids = @voteridlist, positivevote = positivevote + 1 where tid = @tid", BaseConfigs.GetTablePrefix), parms);
            else
                DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("update {0}debates set negativevoterids = @voteridlist, negativevote = negativevote + 1 where tid = @tid", BaseConfigs.GetTablePrefix), parms);
        }
        ...
        /// <param name="tid"></param>
        /// <returns></returns>
        public IDataReader GetTopicPushFeedLog(int tid)
        {
            return DbHelper.ExecuteReader(CommandType.StoredProcedure, string.Format("{0}getfeedlog", BaseConfigs.GetTablePrefix), 
                DbHelper.MakeInParam("@tid", (DbType)SqlDbType.Int, 4, tid));
        }
        ...
        /// <param name="tid"></param>
        /// <returns></returns>
        public int DeleteTopicPushFeedLog(int tid)
        {
            return DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, string.Format("{0}deletefeedlog", BaseConfigs.GetTablePrefix),
                DbHelper.MakeInParam("@tid", (DbType)SqlDbType.Int, 4, tid));
        }
    }
}
