...
namespace Discuz.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        /// <returns></returns>
        public DataTable GetModerForums(int uid)
        { 
            return DbHelper.ExecuteDataset(string.Format("SELECT [fid], [name] FROM {0}forums WHERE [fid] IN (SELECT [fid] FROM {0}moderators WHERE [uid]={1})", BaseConfigs.GetTablePrefix,uid)).Tables[0];
        }
