...
namespace Discuz.Forum
{
    ...
    public class UserAuthority
    {
        ...
        public static bool PostAttachAuthority(ForumInfo forum, UserGroupInfo userGroupInfo, int userId, ref string msg)
        {
            ...
            {
                bool forumAllowapostAttach = Forums.AllowPostAttach(forum.Postattachperm, userGroupInfo.Groupid);
                if(forumAllowapostAttach && forum.Postattachperm != "")
                    return true;
                if (!forumAllowapostAttach)
                {
                    msg = "您没有在该版块上传附件的权限";
                    return false;
                }
        ...
        public static bool Search(int userid, string lastsearchtime, int useradminid, UserGroupInfo usergroupinfo, ref string msg)
        {
            ...
            if (useradminid != 1)
            {
                ...
                if (CreditsFacade.Search(userid) == -1)
                {
