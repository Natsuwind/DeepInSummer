    ...
    public class Forums
    {
        ...
        public static string GetCurrentTopicTypesOption(int fid, string topictypes)
        {
            ...
            if (topictypeoptions == null)
            {
                ...
                foreach (string topictype in topictypes.Replace("\r\n", "").Split('|'))
                {
        ...
        {
            StringBuilder stringbuilder = new StringBuilder();
            foreach (DataRow dr in Discuz.Data.Forums.GetModerForums(uid).Rows)
            {
                stringbuilder.AppendFormat("<option value=\"{0}\">{1}</option>", dr["fid"], dr["name"]);
            }
            return stringbuilder.ToString();
        }
