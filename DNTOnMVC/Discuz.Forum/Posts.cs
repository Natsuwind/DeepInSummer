...
namespace Discuz.Forum
{
    ...
    public class Posts
    {
        ...
        public static int DeletePost(string postTableId, int pid, bool reserveAttach, bool chanagePostStatistic)
        {
            ...
            //}
            PostInfo postInfo = Data.Posts.GetPostInfo(postTableId, pid);
            CreditsFacade.DeletePost(postInfo, Forums.GetForumInfo(postInfo.Fid), reserveAttach);
        ...
        public static List<PostInfo> GetPostList(string tidList)
        {
            if (!Utils.IsNumericList(tidList))
                return null;
            ...
            return Data.Posts.GetPostList(tidList, postTableIdArray);
        }
        ...
        public static List<ShowtopicPagePostInfo> GetPostList(PostpramsInfo postpramsInfo, out List<ShowtopicPageAttachmentInfo> attachList, bool isModer)
        {
            ...
            {
                postInfo.Smileyoff = forumInfo.Allowsmilies == 0 ? 1 : postInfo.Smileyoff;
                postInfo.Bbcodeoff = forumInfo.Allowbbcode == 0 ? 1 : postInfo.Bbcodeoff;
        ...
        public static void LoadExtraPostInfo(ShowtopicPagePostInfo postInfo, int adCount)
        {
        ...
        private static void LoadPostMessage(PostpramsInfo postpramsInfo, List<ShowtopicPageAttachmentInfo> attachList, bool isModer, int allowGetAttach, int originalHideStatus, ShowtopicPagePostInfo postInfo)
        {
            ...
            if (postInfo.Invisible == 1)
                postInfo.Message = "<div class='hintinfo'>该帖子尚未通过审核, 您是发帖者, 以下是帖子内容</div>" + postInfo.Message;
            else
            {
        ...
        public static void AuditPost(int postTableId, string validate, string delete, string ignore, string fidlist)
        {
            ...
            if (!string.IsNullOrEmpty(validate))
            {
                ...
                {
                    CreditsFacade.PostReply(TypeConverter.ObjectToInt(postInfo["posterid"]), Forums.GetForumInfo(TypeConverter.ObjectToInt(postInfo["fid"])));
