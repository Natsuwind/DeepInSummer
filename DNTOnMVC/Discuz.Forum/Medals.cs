...
namespace Discuz.Forum
{
    public class Medals
    {
        ...
        {
            if (medalid > 0)
                Data.Medals.InsertMedal(medalid, name, image);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static void InsertMedalList(System.Collections.ArrayList medalFiles)
        {
            ...
            for (int i = 0; i < medalFiles.Count; i++) //将未入库的勋章入库
            {
                ...
                Medals.InsertMedal(newMedalId, "Medal No." + newMedalId, medalFiles[i].ToString());
            }
            Discuz.Cache.DNTCache.GetCacheService().RemoveObject("/Forum/UI/MedalsList");
        }
    }
