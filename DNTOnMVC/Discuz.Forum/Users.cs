...
namespace Discuz.Forum
{
    ...
    public class Users
    {
        ...
        public static string CheckRegisterDateDiff(string ip)
        {
            ...
            if (config.Regctrl > 0 && userinfo != null)
            {
            ...
            if (config.Ipregctrl.Trim() != "" && Utils.InIPArray(DNTRequest.GetIP(), Utils.SplitString(config.Ipregctrl, "\n")) && userinfo != null)
            {
        ...
        //        //将用户调整到相应的用户组
        //        UserGroupInfo tempGroupInfo = CreditsFacade.GetCreditsUserGroupId(0);
        ...
        public static int SendPMByGroupidList(string groupidlist, int topnumber, ref int start_uid, string msgfrom, int msguid, int folder, string subject, string postdatetime, string message)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //给用户增加新短消息数
                if (OnlineUsers.GetOlidByUid(pm.Msgtoid) > 0)
                    Users.UpdateUserNewPMCount(pm.Msgtoid, OnlineUsers.GetOlidByUid(pm.Msgtoid));
            }
        ...
        public static VerifyRegisterInfo GetVerifyRegisterInfo(string verifyCode)
        {
            ...
            return Data.Users.GetVerifyRegisterInfo(verifyCode, "code");
        }
        ...
        {
            return Data.Users.DeleteVerifyRegisterInfo(regId);
        }
        ...
        {
            return Data.Users.GetVerifyRegisterInfo(ip, "ip");
        }

        /// <summary>
        /// 获取指定Email最近一次的请求验证注册信息
        /// </summary>
        /// <param name="email"></param>
        /// <returns></returns>
        public static VerifyRegisterInfo GetVerifyRegisterInfoByEmail(string email)
        {
            return Data.Users.GetVerifyRegisterInfo(email, "email");
        }
        ...
        /// </summary>
        /// <param name="userName">用户名</param>
        /// <param name="errorMessage">如果不合法,返回的错误信息</param>
        /// <returns></returns>
        public static bool PageValidateUserName(string userName, out string errorMessage)
        {
            if (!IsAvailableType(userName, out errorMessage))
            {
                return false;
            }
            ...
            if (userName.Trim() == PrivateMessages.SystemUserName ||
                     ForumUtils.IsBanUsername(userName, GeneralConfigs.GetConfig().Censoruser))
            {
        ...
        /// <summary>
        /// 验证用户名格式是否正确（不进行数据库已存在和禁止名单校验，如果需要请使用 PageValidateUserName 方法）
        /// </summary>
        /// <param name="userName">用户名</param>
        /// <param name="errorMessage">如果不合法,返回的错误信息</param>
        /// <returns></returns>
        public static bool IsAvailableType(string userName, out string errorMessage)
        {
        ...
        /// </summary>
        /// <param name="email"></param>
        /// <param name="checkVerifyRegister"></param>
        /// <returns></returns>
        public static bool PageValidateEmail(string email, bool checkVerifyRegister,out string errorMessage)
        {
            ...
            if (config.Accessemail.Trim() != "")
            {
                ...
                {
                  errorMessage=  "Email: \"" + email + "\" 不在本论坛允许范围之类, 本论坛只允许用户使用这些Email地址注册: " +
            ...
            if (config.Censoremail.Trim() != "")
            {
                ...
                {
                   errorMessage= "Email: \"" + email + "\" 不允许在本论坛使用, 本论坛不允许用户使用的Email地址包括: " +
