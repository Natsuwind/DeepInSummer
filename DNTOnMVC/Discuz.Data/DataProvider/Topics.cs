...
namespace Discuz.Data
{
    public class Topics
    {
        ...
        {
            Discuz.Common.Generic.List<TopicInfo> list = new Discuz.Common.Generic.List<TopicInfo>();
            IDataReader reader = DatabaseProvider.GetInstance().GetMyUnauditTopic(posterId, tpp, pageId, filter);
            while (reader.Read())
            {
                list.Add(LoadSingleTopicInfo(reader));
            }
        ...
        /// <param name="posterid">作者ID</param>
        /// <param name="filter">-2为未审核</param>
        public static int GetMyUnauditTopicCount(int posterid, int filter)
        {
            return DatabaseProvider.GetInstance().GetMyUnauditTopicCount(posterid, filter);
        }
